class BookmarksProvider {
  ...
  // managing bookmarks in the given editor, or `null` for an editor this
  // package never attached to.
  //
  // Through the map the package already keys by editor rather than by
  // scanning the list: a linear scan matching on `editor.id` returns whichever
  // instance comes first, which is not necessarily the live one.
  getInstanceForEditor(editor) {
    return this.main.bookmarksByEditor.get(editor) ?? null;
  }
}
