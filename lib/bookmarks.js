...
module.exports = class Bookmarks {
  ...
  // The first bookmark starting below `bufferRow`, wrapping round to the top.
  bookmarkAfterRow(bufferRow) {
    const markers = this.markerLayer.getMarkers().sort((a, b) => a.compare(b));
    return markers.find((marker) => marker.getBufferRange().start.row > bufferRow) || markers[0];
  }

  // The first bookmark ending above `bufferRow`, wrapping round to the bottom.
  // Compared on the end row so that a bookmark the caret is sitting inside
  // counts as behind it: anchored on the start row, going back from inside a
  // multi-row bookmark found that same bookmark again.
  bookmarkBeforeRow(bufferRow) {
    const markers = this.markerLayer.getMarkers().sort((a, b) => b.compare(a));
    return markers.find((marker) => marker.getBufferRange().end.row < bufferRow) || markers[0];
  }

  // Where the search starts. The caret, not `getLastCursor().getMarker()
  // .getStartBufferPosition()` — a cursor and its selection share one marker,
  // so that was the top of the selection, and from a selection made downwards
  // it searched from where the selection began rather than from the caret.
  searchRow() {
    return this.editor.getCursorBufferPosition().row;
  }

  jumpToNextBookmark() {
    if (this.markerLayer.getMarkerCount() === 0) return;

    const bookmark = this.bookmarkAfterRow(this.searchRow());
    this.editor.setSelectedBufferRange(bookmark.getBufferRange(), { autoscroll: false });
  ...
  jumpToPreviousBookmark() {
    ...
    const bookmark = this.bookmarkBeforeRow(this.searchRow());
    this.editor.setSelectedBufferRange(bookmark.getBufferRange(), { autoscroll: false });
    this.editor.scrollToCursorPosition();
  }

  // Reach the bookmark from wherever the selection is anchored. Keeping the
  // tail is what lets a second press extend the selection: built from the
  // caret instead, the range collapsed onto itself every other press. The
  // near edge, so a multi-row bookmark is reached rather than swallowed.
  selectToBookmark(bookmark, edge) {
    const tail = this.editor.getLastSelection().getTailBufferPosition();
    const head = bookmark.getBufferRange()[edge];
    this.editor.setSelectedBufferRange([tail, head], {
      // Explicitly, because `Selection#setBufferRange` otherwise inherits the
      // orientation the selection already had.
  ...
  selectToNextBookmark() {
    ...
    this.selectToBookmark(this.bookmarkAfterRow(this.searchRow()), "start");
  }

  selectToPreviousBookmark() {
    if (this.markerLayer.getMarkerCount() === 0) return;

    this.selectToBookmark(this.bookmarkBeforeRow(this.searchRow()), "end");
  }
};
